...
const IncomeEntry: React.FC<IncomeEntryProps> = ({
  ...
  if (loading) {
    return <div className="max-w-4xl mx-auto p-4 md:p-6 space-y-6">
      ...
      <Card className="bg-white/90 backdrop-blur-sm shadow-xl border-0">
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            {incomeFields.map(field => <div key={field.key} className="space-y-2">
                <Label className="text-base md:text-lg font-semibold flex items-center gap-2">
                  <span className="text-xl">{field.icon}</span>
                  {field.label}
                </Label>
                <div className="relative">
                  <Input type="number" min="0" step="0.01" value={income[field.key as keyof Omit<IncomeData, 'total'>] || ''} onChange={e => handleInputChange(field.key as keyof Omit<IncomeData, 'total'>, e.target.value)} placeholder="0.00" className="h-12 md:h-14 text-base md:text-lg pr-12 border-2 border-gray-200 focus:border-blue-500 transition-colors bg-slate-50" />
